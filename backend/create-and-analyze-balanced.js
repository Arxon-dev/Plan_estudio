...
async function createAndAnalyzeBalancedPlan() {
  try {
    console.log('🚀 Creando nuevo plan de prueba con distribución equilibrada...');
    
    // Crear un nuevo plan de estudio con el formato correcto
    const planData = {
      ...
      themes: [
        { id: 1, name: 'Constitución Española - Tema 1', hours: 8, priority: 'HIGH' },
        { id: 2, name: 'Ley Orgánica 5/2005 - Tema 2', hours: 6, priority: 'MEDIUM' },
        { id: 3, name: 'Ley 40/2015 - Tema 3', hours: 10, priority: 'HIGH' },
        { id: 4, name: 'Real Decreto 205/2024 - Tema 4', hours: 4, priority: 'LOW' },
        { id: 5, name: 'Real Decreto 521/2020 - Tema 5', hours: 6, priority: 'MEDIUM' }
      ]
    };
    
    const createResponse = await api.post('/study-plans', planData);
    const newPlan = createResponse.data.plan;
    
    console.log(`✅ Plan creado: ID ${newPlan.id}`);
    
    // Esperar a que se genere el calendario
    console.log('⏳ Esperando generación del calendario...');
    await new Promise(resolve => setTimeout(resolve, 3000));
    
    // Obtener todas las sesiones
    console.log('📚 Obteniendo sesiones del nuevo plan...');
    const sessionsResponse = await api.get(`/study-plans/${newPlan.id}/sessions`);
    ...
    console.log('\n' + '='.repeat(80));
    console.log(`🧹 Limpiando: Eliminando plan de prueba ID ${newPlan.id}...`);
    
    // Eliminar el plan de prueba
    await api.delete(`/study-plans/${newPlan.id}`);
    ...
    console.error('❌ Detalles del error:', error.response?.status, error.response?.statusText);
    if (error.response?.data?.details) {
      console.error('❌ Detalles adicionales:', error.response.data.details);
    }
  }
}

createAndAnalyzeBalancedPlan();