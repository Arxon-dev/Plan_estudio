...
async function checkPlansDirectly() {
  try {
    ...
    const [themeColumns] = await connection.execute('DESCRIBE themes');
    console.log('Columnas de themes:', themeColumns.map(col => col.Field));
    ...
    for (const plan of plans) {
      ...
      if (themes.length > 0) {
        // Obtener nombres de temas - usar la columna correcta
        const themeIds = themes.map(t => t.themeId);
        const [themeNames] = await connection.execute('SELECT id, title, complexity FROM themes WHERE id IN (?)', [themeIds]);
