const { StudyPlan } = require('./backend/src/models');
const { StudyPlanService } = require('./backend/src/services/StudyPlanService');
...
async function verificarDistribucion(planId) {
  try {
    ...
    const plan = await StudyPlan.findByPk(planId, {
      include: [{
        model: require('./backend/src/models').StudySession,
        as: 'sessions',
        include: [{
          model: require('./backend/src/models').Theme,
          as: 'theme'
        }]
      }]
    });
